...
def test_get_all_resources():
    ...
    res1 = skil.resources.compute.HDInsight(sk, "hd_insight_1" + str(uuid.uuid1())[:6],
                                            "test_subscription_id", "test_resource_group_name", "test_cluster_name",
                                            "test_credentials")

    res2 = skil.resources.compute.DataProc(sk, "data_proc_1" + str(uuid.uuid1())[:6],
                                           "test_project_id", "test_region", "test_cluster_name", "test_credentials")
...
def test_get_resources_by_type():
    ...
    res1 = skil.resources.compute.HDInsight(sk, "hd_insight" + str(uuid.uuid1())[:6],
                                            "test_subscription_id", "test_resource_group_name", "test_cluster_name",
                                            "test_credentials")
    res2 = skil.resources.compute.DataProc(sk, "data_proc" + str(uuid.uuid1())[:6],
                                           "test_project_id", "test_region", "test_cluster_name", "test_credentials")

    res3 = skil.resources.compute.EMR(sk, "emr" + str(uuid.uuid1())[:6], "test_region",
                                      "test_credential_uri", "test_cluster_id")

    res4 = skil.resources.storage.AzureStorage(sk, "azure_storage" + str(uuid.uuid1())[:6],
                                               "test_container_name", "test_credentials")

    res5 = skil.resources.storage.GoogleStorage(sk, "google_storage" + str(uuid.uuid1())[:6],
                                                "test_project_id", "test_bucket_name", "test_credentials")

    res6 = skil.resources.storage.S3(sk, "s3" + str(uuid.uuid1())[:6], "test_bucket",
                                     "test_region", "test_credentials")

    res7 = skil.resources.storage.HDFS(sk, "hdfs" + str(uuid.uuid1())[:6], "test_name_node_host",
                                       42, "test_credentials")
    ...
    assert resources[1].resource_id == res5.resource_id
    assert resources[2].resource_id == res6.resource_id
    assert resources[3].resource_id == res7.resource_id
